...
public class FlickeringLight : MonoBehaviour
{
    ...
    [Tooltip(
        "The moves (new targets for properties; intensity, range, position) your light will do per second.")]
    public float speed;
    ...
    private void IntensityAndRange() {
        if (setNewTargets
        ) ////////////////SETTING INTENSITY TARGET IS ENOUGH, AS WE ONLY USE IT TO CHECK IF EVERY PROPERTY HAS REACHED THEIR TARGET. WE CAN DO THIS BECAUSE WE KNOW THEY ALL REACH THEIR TARGETS AT THE SAME TIME.//////
        {
            intensityDelta = (intensityOrigin + Random.Range(-intensityOffset, intensityOffset) -
            ...
            positionDelta =
                (positionOrigin +
                 new Vector3(Random.Range(-1f, 1f), Random.Range(-1f, 1f), Random.Range(-1f, 1f))
                    .normalized * positionOffset - transform.position) * speed;
        }
...
public class MyScriptEditor : Editor
{
    public override void OnInspectorGUI() {
        ...
        flicker.MakeSourceStationary =
            GUILayout.Toggle(flicker.MakeSourceStationary, "Make Source Stationary");

        if (!flicker.MakeSourceStationary)
            flicker.positionOffset = EditorGUILayout.FloatField(
                new GUIContent("Position Offset Multiplier:",
                               "Source's movement freedom. Increasing this value will increase shadow flickering. IF you don't want shadow flickering, check 'Make Source Stationary'."),
                flicker.positionOffset);
    }
}
